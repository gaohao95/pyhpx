...
def create_node(low, high):
    ...
    addrBlock = hpx.AddrBlock.alloc_local_at_sync(1, 1, node_type, 0, hpx.GlobalAddr.locality(where))
    vals = np.array([(hpx.GlobalAddr.NULL, hpx.GlobalAddr.NULL, low, high, (0.0, 0.0, 0.0), hpx.GlobalAddr.NULL, 0)], dtype=node_type)
    localAddr = hpx.LocalAddr(vals.__array_interface__['data'][0])
    localAddr.memput_rsync(addrBlock.addr, node_type.itemsize)
    return addrBlock

def generate_parts(n_parts, domain_length, where):
    partsBlock = hpx.AddrBlock.alloc_local_at_sync(1, n_parts, particle_type, 0, where)
    parts = partsBlock.try_pin()
...
def partition_node_sync(node, parts, n_parts, n_partition):
    ...
    assert done != hpx.GlobalAddr.NULL
    hpx.call(node.addr, partition_node_action, done, parts.addr, n_parts, n_partition)
    hpx.lco_wait(done)
