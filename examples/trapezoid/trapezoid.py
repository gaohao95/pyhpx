...
num_node = 10
cell_per_node = 100
step_size = (high - low) / (num_node * cell_per_node)
...
def main_action():
    ...
    for i in range(num_node):
        calculate_integral(hpx.THERE(i), low + i*step_size*cell_per_node, result_lco)
    print(result_lco.get())
    print(fint(high) - fint(low))
    hpx.exit()

@hpx.create_action()
def calculate_integral(start, result_lco):
    result = 0
